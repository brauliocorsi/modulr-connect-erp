...
BEGIN
  ...
  -- Prefer route assignment; fall back to legacy batch; allow ready standalone deliveries.
  ...
  IF NOT v_is_unassigned AND v_driver IS DISTINCT FROM auth.uid() AND NOT public.has_group(auth.uid(),'system_admin') THEN
    RAISE EXCEPTION 'Esta entrega não está atribuída ao motorista atual';
  END IF;

  IF v_is_unassigned AND NOT (public.has_group(auth.uid(),'delivery_driver') OR public.has_group(auth.uid(),'system_admin')) THEN
    RAISE EXCEPTION 'Apenas motoristas podem concluir entregas avulsas';
  END IF;
