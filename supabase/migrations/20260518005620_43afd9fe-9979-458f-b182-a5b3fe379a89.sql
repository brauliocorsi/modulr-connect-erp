...
-- Best-effort: nuke any leftover orphan route for that zone+date now
DO $$
DECLARE v_zone uuid; v_date date := CURRENT_DATE+1; v_rids uuid[];
BEGIN
  SELECT id INTO v_zone FROM delivery_zones WHERE active ORDER BY created_at LIMIT 1;
  SELECT array_agg(id) INTO v_rids FROM delivery_routes
    WHERE zone_id=v_zone AND route_date=v_date AND state IN ('draft','planned');
  IF v_rids IS NOT NULL THEN
    BEGIN DELETE FROM vehicle_route_manifest WHERE route_id=ANY(v_rids); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN DELETE FROM dock_transfers WHERE route_id=ANY(v_rids); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN DELETE FROM delivery_route_orders WHERE route_id=ANY(v_rids); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN DELETE FROM delivery_routes WHERE id=ANY(v_rids); EXCEPTION WHEN OTHERS THEN NULL; END;
  END IF;
END $$;