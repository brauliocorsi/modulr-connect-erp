-- Mirror vehicle_id from delivery_routes to stock_pickings whenever
-- (a) a schedule/route assignment touches the picking
-- (b) the route's vehicle changes

-- 1) Update tg_delivery_schedules_audit to also mirror vehicle_id from the route
...
-- 2) Update schedule_picking_to_route to propagate vehicle_id
...
-- 3) New trigger on delivery_routes: when vehicle changes, propagate to active pickings + delivery_schedules
CREATE OR REPLACE FUNCTION public.tg_delivery_routes_vehicle_sync()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF TG_OP = 'UPDATE' AND NEW.vehicle_id IS DISTINCT FROM OLD.vehicle_id THEN
    ...
    UPDATE public.delivery_schedules
       SET vehicle_id = NEW.vehicle_id, updated_at = now()
     WHERE route_id = NEW.id
       AND status NOT IN ('cancelled','delivered','rescheduled');
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS tg_delivery_routes_vehicle_sync ON public.delivery_routes;
CREATE TRIGGER tg_delivery_routes_vehicle_sync
AFTER UPDATE ON public.delivery_routes
FOR EACH ROW EXECUTE FUNCTION public.tg_delivery_routes_vehicle_sync();

-- 4) Backfill: align existing active pickings with their route's vehicle
UPDATE public.stock_pickings sp
   SET vehicle_id = dr.vehicle_id
  FROM public.delivery_routes dr
 WHERE sp.route_id = dr.id
   AND sp.vehicle_id IS NULL
   AND dr.vehicle_id IS NOT NULL
   AND sp.state NOT IN ('done','cancelled');

UPDATE public.delivery_schedules ds
   SET vehicle_id = dr.vehicle_id, updated_at = now()
  FROM public.delivery_routes dr
