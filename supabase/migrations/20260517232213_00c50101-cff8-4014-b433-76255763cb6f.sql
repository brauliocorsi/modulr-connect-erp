...
BEGIN
  ...
  BEGIN
    ...
    v_obs := format('mo_estr=%s parent=%s root=%s child=%s',
  ...
  -- A05: capturar TODOS os POs criados (1 por fornecedor)
  BEGIN
    ...
    v_po_id := COALESCE(v_po_ids[1], NULL);
    ...
    v_report := v_report || jsonb_build_object('id','A05','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END,
                                               'observed', format('pos=%s ids=%s', COALESCE(array_length(v_po_ids,1),0), v_po_ids));
    IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
  ...
  -- A06: confirmar TODOS os POs e validar TODOS os pickings incoming
  BEGIN
    ...
    v_obs := format('pos=%s picks=%s/%s', v_pos_done, v_picks_done, v_picks_total);
  ...
  BEGIN
    ...
    BEGIN PERFORM public.close_mo(v_mo_estr, NULL);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
    END;
    v_obs := COALESCE((SELECT state::text FROM manufacturing_orders WHERE id=v_mo_estr),'?');
    v_pass := v_obs = 'done';
    v_obs := 'state='||v_obs || CASE WHEN v_sqlerrm IS NOT NULL THEN ' err='||v_sqlstate||':'||v_sqlerrm ELSE '' END;
    v_report := v_report || jsonb_build_object('id','A08','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END,'observed',v_obs);
    IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  EXCEPTION WHEN OTHERS THEN v_fail:=v_fail+1; END;
  ...
  BEGIN
    v_sqlstate := NULL; v_sqlerrm := NULL;
    BEGIN PERFORM public.close_mo(v_mo_cama, NULL);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
    END;
    ...
    v_pass := v_obs = 'done';
    v_obs := 'state='||v_obs || CASE WHEN v_sqlerrm IS NOT NULL THEN ' err='||v_sqlstate||':'||v_sqlerrm ELSE '' END;
    v_report := v_report || jsonb_build_object('id','A09','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END,'observed',v_obs);
    IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  EXCEPTION WHEN OTHERS THEN v_fail:=v_fail+1; END;
  ...
  BEGIN
    ...
    v_obs := format('mr_raw=%s mr_backed=%s phys=%s qty_ready=%s cls=%s',
  ...
  BEGIN
    ...
    v_obs := format('cls=%s phys=%s mo=%s', v_cls->>'classification',
  ...
  v_gaps := v_gaps
    || jsonb_build_object('id','G_VAR','severity','P2','detail','Variantes não exercitadas.')
    || jsonb_build_object('id','G_PAY','severity','P2','detail','Split 50/50 e pré-pago a documentar.')
    || jsonb_build_object('id','G_DELIV','severity','P1','detail','Cadeia delivery não exercitada; depende A11 verde.');
...
-- Limpar restos antes de re-executar
SELECT public._cleanup_golden_upm();
