...
-- Patch only the offending INSERT via CREATE OR REPLACE of the test (small surgical change)
-- Easier: use ALTER FUNCTION not supported; redefine entire test with code.
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.manufacturing_orders(code,product_id,qty,state,warehouse_id)
      VALUES (v_prefix||'_MO',v_p3,1,'draft',v_wh) RETURNING id INTO v_mo;
