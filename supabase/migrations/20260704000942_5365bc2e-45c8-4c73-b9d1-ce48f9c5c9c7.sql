...
-- Remove all qty_produced references from close_mo (column doesn't exist)
...
BEGIN
  ...
  produced := GREATEST(0, COALESCE(mo.qty, 0));
  ...
  UPDATE public.manufacturing_orders
     SET material_cost = v_material_cost, labor_cost = v_labor_cost,
         total_cost = v_total_cost, unit_cost = v_unit_cost
   WHERE id = _mo;
  ...
  UPDATE public.manufacturing_orders SET state='done', updated_at = now() WHERE id = _mo;
...
-- Update test: remove qty_produced
...
BEGIN
  ...
  VALUES ('MO-TC-'||substr(gen_random_uuid()::text,1,6), v_finished, 1, v_wh, 'confirmed')
  ...
  INSERT INTO public.manufacturing_orders(code, product_id, qty, warehouse_id, state)
  VALUES ('MO-CHILD-'||substr(gen_random_uuid()::text,1,6), v_semi, 1, v_wh, 'confirmed')
  ...
  INSERT INTO public.manufacturing_orders(code, product_id, qty, warehouse_id, state)
  VALUES ('MO-PARENT-'||substr(gen_random_uuid()::text,1,6), v_finished, 1, v_wh, 'confirmed')
  ...
  BEGIN
    ...
    INSERT INTO public.manufacturing_orders(code, product_id, qty, warehouse_id, state)
    VALUES ('MO-ZERO-'||substr(gen_random_uuid()::text,1,6), v_zero_prod, 1, v_wh, 'confirmed')
