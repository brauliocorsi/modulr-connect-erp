...
BEGIN
  ...
  SELECT id INTO v_loc FROM public.stock_locations WHERE type='internal' AND active=true LIMIT 1;
  ...
  v_ok := v_qty_res = 3 OR COALESCE(v_qty_res,0) > 0;
  ...
  v_tests := v_tests || jsonb_build_object('name','10_tracking_on_allocates_available','passed',v_ok,'observed',NULL);
  IF v_ok THEN v_passed:=v_passed+1; ELSE v_failed:=v_failed+1; END IF;
  ...
  -- TRANSFER tests (OFF product l1 -> new line)
  DECLARE v_so_t uuid; v_lt uuid;
  BEGIN
    INSERT INTO public.sale_orders(name,partner_id,state,warehouse_id,company_id) VALUES (v_prefix||'_T',v_partner,'confirmed',v_wh,v_company) RETURNING id INTO v_so_t;
    INSERT INTO public.sale_order_lines(order_id,product_id,quantity,operational_status) VALUES (v_so_t, v_prod_off, 3, 'waiting_purchase') RETURNING id INTO v_lt;
  ...
  BEGIN
    ...
    v_r := public.transfer_sale_reservation(v_l_b, v_l_b3, 2, 'test16');
  ...
  DELETE FROM public.stock_packages WHERE product_id IN (v_prod_off,v_prod_on,v_prod_strict,v_prod_manual,v_prod_paid,v_prod_date,v_prod_custom);
