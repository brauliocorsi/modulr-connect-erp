-- F16-B0.4 — Ajuste de close_mo: MO manual vs MO de venda + stock_packages
...
BEGIN
  ...
  -- Determine case (manual / sale_active / sale_cancelled)
  ...
  ELSE
    ...
    IF so_state IS NULL OR so_state IN ('cancelled') THEN
      v_case := 'sale_cancelled';
    ELSIF so_state IN ('done','completed') THEN
      v_case := 'sale_cancelled';
    ELSE
      v_case := 'sale_active';
    END IF;
  ...
  -- ===== Consume components (unchanged behaviour) =====
  ...
  -- ===== Stock packages (idempotent via deterministic package_ref) =====
  IF v_pkg_tracking THEN
    v_unit_count := CASE
  ...
  IF v_case IN ('manual','sale_cancelled') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- non-fatal: allocation is opportunistic
      NULL;
    END;
  END IF;
