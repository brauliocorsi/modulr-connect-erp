...
DECLARE
  ...
  v_product uuid := '9be30b8e-a281-4cb3-ba7a-7732a1ef75f2';
...
BEGIN
  ...
  -- T15: no stock_moves.package_id leak with stock_packages.id
  ...
  -- T16: every package movement has stock_move_id
  ...
  -- T17: no package on any vehicle stock_location without manifest
  IF NOT EXISTS (
  ...
  -- cleanup: capture move ids first, then delete movements, then moves
  SELECT ARRAY_AGG(stock_move_id) INTO v_move_ids
    FROM stock_package_movements
  ...
  IF v_move_ids IS NOT NULL THEN
    DELETE FROM stock_moves WHERE id = ANY(v_move_ids);
  END IF;
