...
DECLARE
  ...
  v_p_comp uuid; v_p_mo uuid; v_p_t07 uuid;
...
BEGIN
  ...
  BEGIN
    ...
    v_tests := v_tests || jsonb_build_array(jsonb_build_object('test','06_custom_no_weights_fallback','ok',v_ok,'qty',v_qr));
    IF v_ok THEN v_passed:=v_passed+1; ELSE v_failed:=v_failed+1; END IF;
  END;

  -- T07: oldest_order_first prioritizes older SO (use ISOLATED product v_p_t07; stock limited)
  ...
  BEGIN
    ...
    INSERT INTO sale_order_lines(order_id,product_id,quantity,operational_status)
      VALUES (v_so_old,v_p_t07,3,'waiting_purchase') RETURNING id INTO v_l_a;
    INSERT INTO sale_orders(name,partner_id,state,warehouse_id,company_id,created_at)
      VALUES (v_prefix||'_T07B',v_partner,'confirmed',v_wh,v_company,now()-interval '1 day') RETURNING id INTO v_so_new;
    INSERT INTO sale_order_lines(order_id,product_id,quantity,operational_status)
      VALUES (v_so_new,v_p_t07,3,'waiting_purchase') RETURNING id INTO v_l_b;
  ...
  BEGIN
    ...
    INSERT INTO stock_packages(product_id,current_location_id,qty,status,condition) VALUES (v_p_dmg, v_loc, 1, 'available','damaged') RETURNING id INTO v_pkg_d;
    ...
    v_ok := COALESCE(v_qr,0) = 0 AND NOT EXISTS (SELECT 1 FROM stock_packages WHERE id=v_pkg_d AND status='reserved');
  ...
  BEGIN
    ...
    INSERT INTO stock_packages(product_id,current_location_id,qty,status,condition) VALUES (v_p_flow, v_loc, 1, 'at_dock','good') RETURNING id INTO v_pkg_f;
  ...
  BEGIN
    ...
    v_ok := COALESCE(v_qr,0) >= 5;
    v_tests := v_tests || jsonb_build_array(jsonb_build_object('test','16_cancel_run_allocation','ok',v_ok,'qty',v_qr));
    IF v_ok THEN v_passed:=v_passed+1; ELSE v_failed:=v_failed+1; END IF;
  END;

  -- T17: cancel_sale_order release: SO becomes cancelled and release is logged
  DECLARE v_pY uuid; v_soY uuid; v_lY uuid; v_rel_log int;
  BEGIN
    ...
    SELECT count(*) INTO v_rel_log FROM stock_reservation_log
      WHERE origin_id = v_soY AND action='release' AND payload->>'source' LIKE 'cancel_sale_order_%';
    v_ok := (v_res->>'ok')::boolean = true AND EXISTS (SELECT 1 FROM sale_orders WHERE id=v_soY AND state='cancelled') AND v_rel_log > 0;
    v_tests := v_tests || jsonb_build_array(jsonb_build_object('test','17_cancel_release_to_stock','ok',v_ok,'rel_log',v_rel_log));
    IF v_ok THEN v_passed:=v_passed+1; ELSE v_failed:=v_failed+1; END IF;
  END;
  ...
  BEGIN
    ...
    INSERT INTO sale_orders(name,partner_id,state,warehouse_id,company_id,created_at) VALUES (v_prefix||'_T22A',v_partner,'confirmed',v_wh,v_company,now()-interval '2 days') RETURNING id INTO v_other_so;
    INSERT INTO sale_order_lines(order_id,product_id,quantity,operational_status,qty_reserved) VALUES (v_other_so,v_p22,1,'waiting_purchase',1) RETURNING id INTO v_other_l;
    ...
    INSERT INTO stock_packages(product_id,current_location_id,qty,status,condition,sale_order_line_id,sale_order_id) VALUES (v_p22, v_loc, 1, 'reserved','good', v_other_l, v_other_so) RETURNING id INTO v_pkg22;
  ...
  BEGIN
    ...
    v_tests := v_tests || jsonb_build_array(jsonb_build_object('test','23_health_check_runs','ok',v_ok));
    IF v_ok THEN v_passed:=v_passed+1; ELSE v_failed:=v_failed+1; END IF;
  END;
