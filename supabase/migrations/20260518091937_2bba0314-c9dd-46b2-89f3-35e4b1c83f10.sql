...
BEGIN
  ...
  -- Pre-seed: ensure destination quant exists with the receipt qty BEFORE flipping move=done,
  -- simulating the validate_picking pipeline. Then the service trigger only bumps reserved.
  BEGIN
  ...
  -- A10 Not free: same quant must show qty>=2, reserved>=2, available<=0
  BEGIN
    ...
    v_report := v_report || jsonb_build_object('id','A10','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END,'observed',format('Δqty=%s Δreserved=%s Δavailable=%s',v_quant_qty,v_quant_res,COALESCE(v_quant_qty,0)-COALESCE(v_quant_res,0)));
    IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  EXCEPTION WHEN OTHERS THEN GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
  ...
  -- A12 MO done -> reserve (origin_type='MO').
  -- Pre-seed: ensure quant for v_estr exists with the produced qty BEFORE flipping mo=done.
  BEGIN
    SELECT id INTO v_quant_id FROM stock_quants
