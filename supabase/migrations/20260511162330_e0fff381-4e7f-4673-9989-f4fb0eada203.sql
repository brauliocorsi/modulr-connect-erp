...
BEGIN
  ...
  -- Detect any leftover (received < ordered) to create a backorder
  SELECT EXISTS(
  ...
  IF has_leftover THEN
    ...
    INSERT INTO public.stock_pickings(
      ...
      bo_name, cur_picking.kind, 'assigned', cur_picking.warehouse_id,
  ...
  FOR m IN SELECT * FROM public.stock_moves WHERE picking_id = _picking AND state <> 'cancelled' LOOP
    -- Treat NULL as ordered quantity, but respect explicit 0
    ...
    ELSE
      -- Nothing received for this line; cancel original move
      UPDATE public.stock_moves SET state='cancelled', quantity_done = 0 WHERE id = m.id;
    END IF;
    ...
    IF leftover > 0 AND bo_id IS NOT NULL THEN
      INSERT INTO public.stock_moves(
        ...
        m.source_location_id, m.destination_location_id, leftover, 0, 'assigned', m.reference
      );
    END IF;
  END LOOP;
