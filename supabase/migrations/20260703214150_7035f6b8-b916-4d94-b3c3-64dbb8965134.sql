...
BEGIN
  ...
  -- (c) root_sale_order_id fake (sem FK) → so_run_operational_plan falha
  SELECT count(*) INTO v_before FROM sale_operational_plan_log WHERE mode='mo_done_failed';
  INSERT INTO manufacturing_orders(code, product_id, qty, uom_id, warehouse_id, state, origin, root_sale_order_id)
  ...
  IF v_after <= v_before THEN
    RAISE EXCEPTION 'FAIL (c): log mo_done_failed não registado (before=%, after=%)', v_before, v_after;
  END IF;
  IF (SELECT state::text FROM manufacturing_orders WHERE id=v_mo) <> 'done' THEN
    RAISE EXCEPTION 'FAIL (c): MO não fechou';
  END IF;
