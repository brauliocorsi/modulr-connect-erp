...
DECLARE
  ...
  v_need_id uuid; v_supplier uuid;
  v_remaining numeric; v_unit_price numeric; v_uom uuid;
  v_has_variant boolean; v_seq int;
  v_po_id uuid; v_pol_id uuid;
  v_results jsonb := '[]'::jsonb;
  v_distinct_suppliers int;
  v_existing_pol uuid; v_existing_po uuid; v_existing_state text;
...
BEGIN
  ...
  FOR v_supplier IN SELECT DISTINCT supplier_id FROM _pn_buf LOOP
    ...
    VALUES (
      ...
      v_supplier, 'draft'::purchase_state,
    ...
    LOOP
      SELECT remaining, unit_price, uom_id INTO v_remaining, v_unit_price, v_uom
