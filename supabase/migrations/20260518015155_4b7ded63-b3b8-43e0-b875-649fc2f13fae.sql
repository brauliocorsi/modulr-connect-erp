...
BEGIN
  ...
  SELECT ug.user_id INTO v_user
    ...
    WHERE g.code='inventory_user'
    ORDER BY ug.user_id LIMIT 1;
  IF v_user IS NULL THEN
    RAISE EXCEPTION 'TEST_SETUP: no user with inventory_user group available';
  END IF;
  PERFORM set_config('request.jwt.claims', json_build_object('sub', v_user::text)::text, true);

  -- PATCH P03: must use the SAME register the trigger tg_payment_register_cash_movement
  -- will pick (oldest active register owned by v_user). Open a test session on it,
  -- so the trigger finds the open session and creates the cash_movement.
  SELECT id INTO v_register FROM cash_registers
    WHERE user_id=v_user AND active ORDER BY created_at ASC LIMIT 1;
  ...
  v_report := v_report || jsonb_build_object('id','SETUP','status','OK',
    'observed', format('register=%s session=%s user=%s', v_register, v_session, v_user));
  v_ok := v_ok + 1;
