...
BEGIN
  ...
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved) VALUES (v_so_new,v_pB,1,0) RETURNING id INTO v_sol_new;
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_SRC',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_src;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved) VALUES (v_so_src,v_pB,1,1) RETURNING id INTO v_sol_src;
  INSERT INTO public.stock_packages(product_id,sale_order_id,sale_order_line_id,qty,status,condition,current_location_id)
  ...
  -- 20
  SELECT count(*) INTO v_neg FROM public.stock_quants WHERE product_id IN (v_pA,v_pB,v_pC,v_pD,v_pE) AND reserved_quantity < 0;
  v_tests := v_tests || jsonb_build_object('test','20_no_negative_reserved','ok',v_neg=0,'detail',jsonb_build_object('count',v_neg));

  -- 21
  SELECT count(*) INTO v_inv FROM public.stock_quants WHERE product_id IN (v_pA,v_pB,v_pC,v_pD,v_pE) AND reserved_quantity > quantity;
