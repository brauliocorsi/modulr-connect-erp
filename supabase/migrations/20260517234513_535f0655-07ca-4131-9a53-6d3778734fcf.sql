...
BEGIN
  ...
  BEGIN
    SELECT count(*) INTO v_leaf_count FROM purchase_needs WHERE product_id = ANY(v_leaf_expected)
      AND state IN ('pending','quoting','approved') AND manufacturing_order_id IS NOT NULL;
  ...
  BEGIN
    SELECT array_agg(id) INTO v_pn_ids FROM purchase_needs WHERE product_id = ANY(v_leaf_expected) AND state IN ('pending','quoting','approved');
    ...
    SELECT array_agg((c->>'purchase_order_id')::uuid) INTO v_po_ids
      FROM jsonb_array_elements(v_rec->'created') c WHERE c->>'purchase_order_id' IS NOT NULL;
      ...
      FOR v_po_id IN SELECT unnest(v_po_ids) LOOP
        ...
        FOR v_pick IN SELECT id FROM stock_pickings WHERE kind='incoming' AND origin = v_po_name LOOP
          ...
          BEGIN
            ...
            IF (SELECT state::text FROM stock_pickings WHERE id=v_pick) = 'done' THEN v_picks_done := v_picks_done + 1; END IF;
  ...
  v_pass := NOT EXISTS (SELECT 1 FROM (SELECT manufacturing_order_id, product_id, COALESCE(product_variant_id,'00000000-0000-0000-0000-000000000000'::uuid) v, count(*) c
    FROM purchase_needs WHERE product_id = ANY(v_leaf_expected) GROUP BY 1,2,3) x WHERE c > 1);
  v_report := v_report || jsonb_build_object('id','A15','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END,'observed','');
  IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;

  v_pass := NOT EXISTS (SELECT 1 FROM (SELECT sale_order_line_id, count(*) c FROM manufacturing_orders
  ...
  BEGIN
    v_cls := public.so_classify_line(v_sol);
    v_pass := NOT ((v_cls->>'classification') = 'ready_stock'
  ...
  BEGIN
    ...
    v_pass := v_obs IN ('done','completed','sale','confirmed');
    v_report := v_report || jsonb_build_object('id','P08','status',CASE WHEN v_pass THEN 'OK' ELSE 'GAP_P2' END,'observed','so_state='||v_obs);
    IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  EXCEPTION WHEN OTHERS THEN GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
    v_report := v_report || jsonb_build_object('id','P08','status','GAP_P2','observed',v_sqlstate||':'||v_sqlerrm); v_fail:=v_fail+1; END;
