...
BEGIN
  ...
  -- Snapshot and deactivate any existing active registers for this user
  SELECT array_agg(id) INTO prev_active FROM public.cash_registers WHERE user_id=v_user AND active=true;
  UPDATE public.cash_registers SET active=false WHERE user_id=v_user AND active=true;
  ...
  -- Restore previously active registers
  IF prev_active IS NOT NULL THEN
    UPDATE public.cash_registers SET active=true WHERE id = ANY(prev_active);
  END IF;
