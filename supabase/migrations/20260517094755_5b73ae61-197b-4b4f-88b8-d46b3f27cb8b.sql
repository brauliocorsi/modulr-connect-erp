...
BEGIN
  ...
  FOR v_row IN
    SELECT mc.id AS mc_id, mc.mo_id, mc.product_id, mc.qty_required, mc.qty_reserved, mo.code
     ...
     WHERE mo.state IN ('waiting_material','ready','in_progress','paused','qc')
  ...
  -- P0.4 component_reserved_quantity_exceeds_stock
  FOR v_row IN
     ...
     WHERE mo.state IN ('waiting_material','ready','in_progress','paused','qc')
       ...
       AND EXISTS (
         ...
         WHERE sol.product_id = mc.product_id
           ...
           AND so.state IN ('confirmed','draft')
       )
  LOOP
     ...
     WHERE p.active = true
       ...
       AND NOT EXISTS (
         ...
         WHERE mc.product_id = p.id
           AND mc.qty_required > COALESCE(mc.qty_reserved,0)
           AND mo.state IN ('waiting_material','ready','in_progress','paused','qc')
       )
  LOOP
