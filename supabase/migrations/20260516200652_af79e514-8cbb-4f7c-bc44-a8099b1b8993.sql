...
DECLARE
  ...
  v_pA uuid; v_pB uuid; v_pC uuid; v_pD uuid; v_pE uuid;
  v_so_legacy uuid; v_sol_legacy uuid;
  v_so2 uuid; v_sol2 uuid;
  v_so3 uuid; v_sol3 uuid;
  v_so4 uuid; v_sol4 uuid;
  v_so5 uuid; v_sol5 uuid;
  v_so6 uuid; v_sol6 uuid;
  v_so7 uuid; v_sol7 uuid;
  v_so_target uuid; v_sol_target uuid;
  v_so_alt uuid; v_sol_alt uuid;
  v_pkg uuid; v_pkg2 uuid; v_pkg_now uuid; v_pkg_dmg uuid;
  v_r jsonb; v_state text; v_status text;
  v_cnt int; v_neg bigint; v_inv bigint;
  v_dec_count int; v_log_count int;
  v_passed int := 0; v_failed int := 0;
  v_err text; v_comp uuid;
...
BEGIN
  ...
  INSERT INTO public.stock_quants(product_id,location_id,quantity,reserved_quantity)
    VALUES (v_pA,v_loc,100,0),(v_pB,v_loc,0,0),(v_pC,v_loc,0,0),(v_pD,v_loc,0,0),(v_pE,v_loc,0,0);

  -- TEST 1: legacy signature
  BEGIN
    INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
      VALUES (v_prefix||'_LEG',v_partner,'confirmed','pending',v_wh,v_company)
      RETURNING id INTO v_so_legacy;
  ...
  v_tests := v_tests || jsonb_build_object('test','4_release_to_stock','ok',
    v_state='cancelled' AND jsonb_array_length(COALESCE(v_r->'reallocation','[]'::jsonb))=0,
    'detail',jsonb_build_object('state',v_state,'r',v_r));

  -- TEST 5: decision_required cria decisão
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_S5',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so5;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so5,v_pD,2,0) RETURNING id INTO v_sol5;
  v_r := public.cancel_sale_order(v_so5, jsonb_build_object('reservation_action','decision_required'));
  SELECT count(*) INTO v_dec_count FROM public.allocation_decisions
   WHERE (payload->>'sale_order_id')::uuid = v_so5 AND state='pending';
  v_tests := v_tests || jsonb_build_object('test','5_decision_required','ok',v_dec_count>=1,
    'detail',jsonb_build_object('count',v_dec_count));

  -- TEST 6: at_dock bloqueia
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_S6',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so6;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so6,v_pB,1,1) RETURNING id INTO v_sol6;
  ...
  v_tests := v_tests || jsonb_build_object('test','6_at_dock_blocks','ok',
    v_state<>'cancelled' AND (v_r->>'state')='decision_required',
    'detail',jsonb_build_object('state',v_state));

  -- TEST 7: loaded bloqueia
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_S7',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so7;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so7,v_pB,1,1) RETURNING id INTO v_sol7;
  INSERT INTO public.stock_packages(product_id,sale_order_id,sale_order_line_id,qty,status,condition,current_location_id)
  ...
  v_tests := v_tests || jsonb_build_object('test','7_loaded_blocks','ok',v_state<>'cancelled',
    'detail',jsonb_build_object('state',v_state));

  -- TEST 8: delivered bloqueia
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_SDLV',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_dlv;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_dlv,v_pB,1,1) RETURNING id INTO v_sol_dlv;
  INSERT INTO public.stock_packages(product_id,sale_order_id,sale_order_line_id,qty,status,condition,current_location_id)
  ...
  v_tests := v_tests || jsonb_build_object('test','8_delivered_blocks','ok',v_state<>'cancelled',
    'detail',jsonb_build_object('state',v_state));

  -- TEST 9: damaged cria decision sem realocar
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_SDMG',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_dmg;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_dmg,v_pB,1,1) RETURNING id INTO v_sol_dmg;
  INSERT INTO public.stock_packages(product_id,sale_order_id,sale_order_line_id,qty,status,condition,current_location_id)
    VALUES (v_pB,v_so_dmg,v_sol_dmg,1,'reserved','damaged',v_loc) RETURNING id INTO v_pkg_dmg;
  v_r := public.cancel_sale_order(v_so_dmg);
  SELECT count(*) INTO v_dec_count FROM public.allocation_decisions
   WHERE (payload->>'package_id')::uuid = v_pkg_dmg;
  v_tests := v_tests || jsonb_build_object('test','9_damaged_decision','ok',v_dec_count>=1,
    'detail',jsonb_build_object('count',v_dec_count));

  -- TEST 10: tracking OFF release reduz reserved_quantity
  ...
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_t,v_pA,2,2) RETURNING id INTO v_sol_t;
  UPDATE public.stock_quants SET reserved_quantity=reserved_quantity+2 WHERE product_id=v_pA AND location_id=v_loc;
  ...
  v_tests := v_tests || jsonb_build_object('test','10_tracking_off_release','ok',v_after_resv<=v_before_resv+2,
    'detail',jsonb_build_object('before',v_before_resv,'after',v_after_resv));

  -- TEST 11: tracking ON safe package -> available
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_SPK',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_pk;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_pk,v_pB,1,1) RETURNING id INTO v_sol_pk;
  INSERT INTO public.stock_packages(product_id,sale_order_id,sale_order_line_id,qty,status,condition,current_location_id)
  ...
  v_tests := v_tests || jsonb_build_object('test','11_tracking_on_release','ok',v_status='available',
    'detail',jsonb_build_object('status',v_status));

  -- TEST 12: tracking ON run_allocation realoca para nova SO
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_NEW',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_new;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_new,v_pB,1,0) RETURNING id INTO v_sol_new;
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_SRC',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_src;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_src,v_pB,1,1) RETURNING id INTO v_sol_src;
  INSERT INTO public.stock_packages(product_id,sale_order_id,sale_order_line_id,qty,status,condition,current_location_id)
    VALUES (v_pB,v_so_src,v_sol_src,1,'reserved','good',v_loc) RETURNING id INTO v_pkg_r;
  ...
  v_tests := v_tests || jsonb_build_object('test','12_realloc_pkg_to_new_so','ok',v_pkg_now_so=v_so_new,
    'detail',jsonb_build_object('new_so',v_pkg_now_so,'expected',v_so_new,'r',v_r));

  -- TEST 13: MO draft cancelada
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_MD',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_md;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_md,v_pE,1,0) RETURNING id INTO v_sol_md;
  ...
  v_tests := v_tests || jsonb_build_object('test','13_mo_draft_cancelled','ok',v_state='cancelled',
    'detail',jsonb_build_object('mo_state',v_state));

  -- TEST 14: MO in_progress strict -> decision_required
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_MI',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_mi;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_mi,v_pD,1,0) RETURNING id INTO v_sol_mi;
  INSERT INTO public.manufacturing_orders(code,product_id,qty,state,warehouse_id,origin,sale_order_id,sale_order_line_id)
  ...
  SELECT count(*) INTO v_dec_count FROM public.allocation_decisions
   WHERE (payload->>'manufacturing_order_id')::uuid = v_mo_inprog;
  v_tests := v_tests || jsonb_build_object('test','14_mo_inprog_strict','ok',
    v_state='in_progress' AND v_dec_count>=1,
    'detail',jsonb_build_object('mo_state',v_state,'dec',v_dec_count));

  -- TEST 15: MO in_progress pool -> desvincula
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_MP',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_mp;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_mp,v_pE,1,0) RETURNING id INTO v_sol_mp;
  INSERT INTO public.manufacturing_orders(code,product_id,qty,state,warehouse_id,origin,sale_order_id,sale_order_line_id)
  ...
  v_tests := v_tests || jsonb_build_object('test','15_mo_inprog_pool_detached','ok',
    v_mo_so IS NULL AND v_state='in_progress',
    'detail',jsonb_build_object('mo_so',v_mo_so,'state',v_state));

  -- TEST 16: MO done -> SO cancelled, MO permanece done
  INSERT INTO public.sale_orders(name,partner_id,state,fulfillment_status,warehouse_id,company_id)
    VALUES (v_prefix||'_MDN',v_partner,'confirmed','pending',v_wh,v_company) RETURNING id INTO v_so_mdone;
  INSERT INTO public.sale_order_lines(order_id,product_id,quantity,qty_reserved)
    VALUES (v_so_mdone,v_pE,1,0) RETURNING id INTO v_sol_mdone;
  ...
  v_tests := v_tests || jsonb_build_object('test','16_mo_done_so_cancelled','ok',
    v_mo_st='done' AND v_state='cancelled',
    'detail',jsonb_build_object('mo',v_mo_st,'so',v_state));

  -- TEST 17: idempotência
  v_r := public.cancel_sale_order(v_so4, jsonb_build_object('reservation_action','release_to_stock'));
  v_tests := v_tests || jsonb_build_object('test','17_idempotent','ok',
    (v_r->>'ok')::boolean = true AND (v_r->>'idempotent')::boolean = true,
    'detail',v_r);

  -- TEST 18: log decision_required
  SELECT count(*) INTO v_log_count FROM public.stock_reservation_log
   WHERE origin_type='SO' AND origin_id=v_so5 AND action='decision_required';
  v_tests := v_tests || jsonb_build_object('test','18_log_decision_required','ok',v_log_count>=1,
    'detail',jsonb_build_object('count',v_log_count));

  -- TEST 19: timeline event
  SELECT count(*) INTO v_log_count FROM public.record_events
   WHERE record_type='sale_order' AND record_id=v_so2;
  v_tests := v_tests || jsonb_build_object('test','19_timeline_event','ok',v_log_count>=1,
    'detail',jsonb_build_object('count',v_log_count));

  -- TEST 20: nenhum reserved_quantity negativo
  SELECT count(*) INTO v_neg FROM public.stock_quants
   WHERE product_id IN (v_pA,v_pB,v_pC,v_pD,v_pE) AND reserved_quantity < 0;
  v_tests := v_tests || jsonb_build_object('test','20_no_negative_reserved','ok',v_neg=0,
    'detail',jsonb_build_object('count',v_neg));

  -- TEST 21: reserved <= quantity
  SELECT count(*) INTO v_inv FROM public.stock_quants
   WHERE product_id IN (v_pA,v_pB,v_pC,v_pD,v_pE) AND reserved_quantity > quantity;
  v_tests := v_tests || jsonb_build_object('test','21_reserved_le_quantity','ok',v_inv=0,
    'detail',jsonb_build_object('count',v_inv));

  -- TEST 22: nenhum package reservado a SO cancelada (fora fluxo físico)
  SELECT count(*) INTO v_cnt
    FROM public.stock_packages p
    JOIN public.sale_orders so ON so.id = p.sale_order_id
  ...
  v_tests := v_tests || jsonb_build_object('test','22_no_pkg_reserved_to_cancelled','ok',v_cnt=0,
    'detail',jsonb_build_object('count',v_cnt));

  -- TEST 23: packages em fluxo físico não realocados
  SELECT count(*) INTO v_cnt FROM public.stock_packages
   WHERE id IN (v_pkg, v_pkg2) AND sale_order_id IS NULL;
  v_tests := v_tests || jsonb_build_object('test','23_physical_pkg_not_realloc','ok',v_cnt=0,
    'detail',jsonb_build_object('count',v_cnt));

  -- TEST 24: nenhum stock_move novo por cancelamento lógico em produtos tracking ON
  SELECT count(*) INTO v_cnt FROM public.stock_moves sm
    WHERE sm.created_at >= now() - interval '5 minutes'
      AND sm.product_id IN (v_pB,v_pC,v_pD,v_pE)
      AND sm.state NOT IN ('cancelled');
  v_tests := v_tests || jsonb_build_object('test','24_no_new_stock_move_tracking_on','ok',v_cnt=0,
    'detail',jsonb_build_object('count',v_cnt));
