...
DECLARE
  ...
  v_pn_ids uuid[]; v_po_id uuid; v_po_name text; v_rec jsonb; v_pick uuid;
...
BEGIN
  ...
  BEGIN
    ...
    v_report := v_report || jsonb_build_object('id','A05','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END,
                                               'observed','po='||COALESCE(v_po_id::text,'NULL'));
    IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
  ...
  -- A06 confirm PO + validar picking incoming
  BEGIN
    IF v_po_id IS NOT NULL THEN
      SELECT name INTO v_po_name FROM purchase_orders WHERE id=v_po_id;
      BEGIN PERFORM public.confirm_purchase_order(v_po_id); EXCEPTION WHEN OTHERS THEN NULL; END;
      SELECT id INTO v_pick FROM stock_pickings
        WHERE kind='incoming' AND origin = v_po_name
  ...
  -- A08: usa RPC oficial close_mo e propaga diagnóstico
  BEGIN
    v_sqlstate := NULL; v_sqlerrm := NULL;
    BEGIN
      PERFORM public.close_mo(v_mo_estr, NULL);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
    END;
    v_obs := COALESCE((SELECT state::text FROM manufacturing_orders WHERE id=v_mo_estr),'?');
    v_pass := v_obs = 'done';
    IF NOT v_pass AND v_sqlerrm IS NOT NULL THEN
      v_obs := 'state='||v_obs||' err='||v_sqlstate||':'||v_sqlerrm;
    ELSE
      v_obs := 'state='||v_obs;
    END IF;
  ...
  -- A09: idem para MO mãe
  BEGIN
    v_sqlstate := NULL; v_sqlerrm := NULL;
    BEGIN
      PERFORM public.close_mo(v_mo_cama, NULL);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_sqlstate=RETURNED_SQLSTATE, v_sqlerrm=MESSAGE_TEXT;
    END;
