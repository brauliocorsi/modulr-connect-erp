...
begin
  ...
  for l in select * from public.sale_order_lines where order_id = _order loop
    ...
    values (v_picking_id, l.product_id, l.variant_id, l.uom_id, src, dst, l.quantity, 'draft'::move_state, o.name);
  end loop;
...
begin
  ...
  for l in select * from public.purchase_order_lines where order_id = _order loop
    insert into public.stock_moves(picking_id, product_id, variant_id, uom_id, source_location_id, destination_location_id, quantity, state, reference)
    values (v_picking_id, l.product_id, l.variant_id, l.uom_id, src, dst, l.quantity,'ready'::move_state, o.name);
  end loop;
...
begin
  ...
  update public.stock_moves
     set state = case when reserved_total >= m.quantity then 'ready'::move_state else 'waiting'::move_state end
   where id = _move;
