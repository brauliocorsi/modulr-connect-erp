CREATE OR REPLACE FUNCTION public._test_phase16_c3_make_incoming_done(
  _product uuid, _src uuid, _dst uuid, _qty numeric,
  _po_line uuid DEFAULT NULL, _need uuid DEFAULT NULL, _ref text DEFAULT NULL
) RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $fn$
DECLARE v_pick uuid; v_move uuid;
BEGIN
  INSERT INTO stock_pickings(name, kind, state, source_location_id, destination_location_id, origin)
  VALUES (COALESCE(_ref,'F16C3_pick_'||substr(gen_random_uuid()::text,1,8)),
          'incoming'::picking_kind, 'done'::picking_state, _src, _dst, 'F16C3')
  RETURNING id INTO v_pick;
  INSERT INTO stock_moves(picking_id, product_id, source_location_id, destination_location_id,
                          quantity, quantity_done, state, purchase_order_line_id, purchase_need_id, reference)
  VALUES (v_pick, _product, _src, _dst, _qty, _qty, 'done'::picking_state, _po_line, _need,
          COALESCE(_ref,'F16C3_mv'))
...
BEGIN
  ...
  SELECT id INTO v_pn_A FROM purchase_needs WHERE mo_component_id=v_mc_A LIMIT 1;
  ...
  END IF;
  SELECT id INTO v_pn_B FROM purchase_needs WHERE mo_component_id=v_mc_B LIMIT 1;
  ...
  IF v_pn_amb1 IS NULL THEN
    ...
    SELECT v_comp_A,1,'manufacturing',id,v_mo_amb1,'pending',v_pol_amb,v_po FROM mo_components WHERE mo_id=v_mo_amb1 AND product_id=v_comp_A
    RETURNING id INTO v_pn_amb1;
  ELSE
    UPDATE purchase_needs SET purchase_order_line_id=v_pol_amb, purchase_order_id=v_po WHERE id=v_pn_amb1;
  END IF;
  IF v_pn_amb2 IS NULL THEN
    INSERT INTO purchase_needs(product_id,qty_needed,origin_kind,mo_component_id,manufacturing_order_id,state,purchase_order_line_id,purchase_order_id)
    SELECT v_comp_A,1,'manufacturing',id,v_mo_amb2,'pending',v_pol_amb,v_po FROM mo_components WHERE mo_id=v_mo_amb2 AND product_id=v_comp_A
    RETURNING id INTO v_pn_amb2;
  ELSE
    UPDATE purchase_needs SET purchase_order_line_id=v_pol_amb, purchase_order_id=v_po WHERE id=v_pn_amb2;
  END IF;
  ...
  -- 19-22 already satisfied
  UPDATE purchase_needs SET satisfied_by='other_purchase', satisfied_at=now(), qty_needed=1 WHERE id = v_pn_amb1;
  UPDATE mo_components SET qty_reserved = qty_required
   WHERE id = (SELECT mo_component_id FROM purchase_needs WHERE id=v_pn_amb1);
  ...
  test_name := '22_already_satisfied_routed_to_engine'; passed := v_res->>'route' IN ('component_allocation_engine','sales_allocation_engine','stock_free_no_allocation'); detail := COALESCE(v_res->>'route',''); RETURN NEXT;
  ...
  BEGIN
    DELETE FROM stock_reservation_log WHERE payload->>'source' = 'mfg_reserve_components_on_receipt'
      AND product_id IN (v_comp_A,v_comp_B,v_comp_C,v_comp_D,v_regular,v_finished,v_finished2,v_finished3);
  ...
  EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN
    DELETE FROM stock_quants WHERE product_id IN (v_comp_A,v_comp_B,v_comp_C,v_comp_D,v_regular,v_finished,v_finished2,v_finished3);
  EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN
    DELETE FROM purchase_needs WHERE product_id IN (v_comp_A,v_comp_B,v_comp_C,v_comp_D,v_regular,v_finished,v_finished2,v_finished3);
  EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN
    DELETE FROM bom_lines WHERE bom_id IN (v_bom,v_bom2,v_bom3);
    DELETE FROM boms WHERE id IN (v_bom,v_bom2,v_bom3);
  EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN
