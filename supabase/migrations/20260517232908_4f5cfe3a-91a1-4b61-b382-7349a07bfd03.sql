...
BEGIN
  IF NEW.state='done' AND COALESCE(OLD.state::text,'') <> 'done' THEN
    ...
    IF v_owner_so IS NOT NULL THEN
      -- mo_done bypassa throttle de replan (evento intrínseco, não loop)
      BEGIN PERFORM so_run_operational_plan(v_owner_so,'mo_done'); EXCEPTION WHEN OTHERS THEN NULL; END;
...
-- Ajustar throttle: só aplica a 'auto' e 'replan', NÃO a 'mo_done' nem outros eventos intrínsecos
...
BEGIN
  ...
  -- Throttle apenas para chamadas regulares; eventos intrínsecos (mo_done, etc.) passam sempre
