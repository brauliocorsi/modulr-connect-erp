...
BEGIN
  FOR v_row IN
    SELECT sol.id, sol.order_id, sol.product_id, sol.qty_reserved, so.name AS so_name
  ...
  FOR v_row IN
    SELECT sp.id, sp.status, so.name AS so_name FROM stock_packages sp JOIN sale_orders so ON so.id = sp.sale_order_id
  ...
  FOR v_row IN SELECT id, product_id, quantity, reserved_quantity FROM stock_quants WHERE reserved_quantity > quantity + 1e-9 LOOP
  ...
  FOR v_row IN
    ...
    WHERE mo.state = 'done' AND so.state = 'confirmed'
  ...
  FOR v_row IN
    ...
    WHERE p.package_tracking_enabled = true AND COALESCE(sol.qty_reserved,0) > 0 AND so.state = 'confirmed'
  ...
  FOR v_row IN
    ...
    WHERE so.state = 'confirmed' AND COALESCE(sol.qty_reserved,0) < sol.quantity - 1e-9
  ...
  FOR v_row IN SELECT id, created_at FROM allocation_decisions WHERE state='pending' AND created_at < v_threshold LOOP
