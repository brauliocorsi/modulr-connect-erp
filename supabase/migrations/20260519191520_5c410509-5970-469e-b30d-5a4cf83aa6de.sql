...
BEGIN
  ...
  IF _cleanup THEN
    -- Activity events generated by triggers
    DELETE FROM public.activity_events
     WHERE (entity_type='customer_ticket' AND entity_id IN (SELECT id FROM public.customer_tickets WHERE customer_id IN (v_cust, v_cust2)))
        OR (entity_type='service_case'    AND entity_id IN (SELECT id FROM public.service_cases   WHERE customer_id IN (v_cust, v_cust2)))
        OR (entity_type='sale_order'      AND entity_id IN (v_so, v_so2));
    -- Ticket attachments + messages
    DELETE FROM public.customer_ticket_attachments
     WHERE ticket_id IN (SELECT id FROM public.customer_tickets WHERE customer_id IN (v_cust, v_cust2));
    DELETE FROM public.customer_ticket_messages
     WHERE ticket_id IN (SELECT id FROM public.customer_tickets WHERE customer_id IN (v_cust, v_cust2));
    -- Tickets first (FK to service_cases via service_case_id)
    UPDATE public.customer_tickets SET service_case_id = NULL WHERE customer_id IN (v_cust, v_cust2);
    DELETE FROM public.customer_tickets WHERE customer_id IN (v_cust, v_cust2);
    -- Service cases
    DELETE FROM public.service_cases WHERE customer_id IN (v_cust, v_cust2);
    -- Portal tokens
    DELETE FROM public.customer_portal_tokens WHERE customer_id IN (v_cust, v_cust2);
    -- Sale orders + lines
    DELETE FROM public.sale_order_lines WHERE order_id IN (v_so, v_so2);
    DELETE FROM public.sale_orders WHERE id IN (v_so, v_so2);
    -- Finally partners
