...
CREATE OR REPLACE FUNCTION public.create_purchase_need(
  ...
  _sale uuid DEFAULT NULL::uuid, _mo uuid DEFAULT NULL::uuid,
  _needed_by date DEFAULT NULL::date, _notes text DEFAULT NULL::text,
  _variant uuid DEFAULT NULL::uuid
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  _id uuid; _supplier uuid; _po_name text;
  _dbg_po_rows int; _dbg_po_all jsonb;
BEGIN
  IF _qty IS NULL OR _qty <= 0 THEN RETURN NULL; END IF;

  IF _sale IS NOT NULL THEN
    -- DBG: list ALL POLs for this sale
    SELECT count(*), jsonb_agg(jsonb_build_object('po_id',po.id,'po_state',po.state,'pol_prod',pol.product_id,'pol_var',pol.variant_id,'src_so',pol.source_sale_order_id))
      INTO _dbg_po_rows, _dbg_po_all
      FROM public.purchase_order_lines pol JOIN public.purchase_orders po ON po.id=pol.order_id
     WHERE pol.source_sale_order_id = _sale;
    RAISE NOTICE 'PO-guard input: sale=% product=% variant=% -> POLs total=% details=%',
      _sale, _product, _variant, _dbg_po_rows, _dbg_po_all;
    ...
    RAISE NOTICE 'PO-guard match name=%', _po_name;
