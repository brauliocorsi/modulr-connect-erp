...
BEGIN
  ...
  -- (b) variant_id no move de reserva (produto com variantes; sem has_variants column)
  ...
  VALUES (v_tag||' PROD-B','storable',true,false,v_uom,'buy'::product_supply_route)
  RETURNING id INTO v_product;
  ...
  SELECT variant_id INTO v_move_variant
    FROM stock_moves sm
  ...
  IF v_res_a < 2 THEN
    RAISE EXCEPTION 'FAIL (b): quant da variante A não reservou (res=%)', v_res_a;
  END IF;
  IF v_res_b <> 0 THEN
    RAISE EXCEPTION 'FAIL (b): quant da variante B foi tocada (res=%)', v_res_b;
  END IF;
