...
BEGIN
  ...
  INSERT INTO products(name, type) VALUES (v_pfx||'PROD','storable') RETURNING id INTO v_prod;
  ...
  v_pass := (v_res->>'ok')::bool AND v_credit IS NOT NULL AND EXISTS(SELECT 1 FROM customer_credits WHERE id=v_credit AND remaining_amount=100.00 AND state='open');
  ...
  v_report:=v_report||jsonb_build_object('id','04_credit_prevent_overapply','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END);
  IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;

  v_res := apply_customer_credit(v_credit, 10.00, v_so_cancelled, NULL, NULL);
  v_pass := v_res->>'error' = 'sale_order_cancelled';
  v_report:=v_report||jsonb_build_object('id','05_credit_blocks_cancelled_so','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END);
  IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;
  ...
  v_pass := (v_res->>'ok')::bool AND (SELECT amount_paid FROM supplier_bills WHERE id=v_bill) = 10.00 AND (SELECT state FROM supplier_bills WHERE id=v_bill) = 'partial';
  ...
  v_report:=v_report||jsonb_build_object('id','14_supplier_overpayment_blocked','status',CASE WHEN v_pass THEN 'OK' ELSE 'FAIL' END);
  IF v_pass THEN v_ok:=v_ok+1; ELSE v_fail:=v_fail+1; END IF;

  v_res := supplier_payment_cancel(v_pay, 'erro');
  v_pass := (v_res->>'ok')::bool AND (SELECT amount_paid FROM supplier_bills WHERE id=v_bill) = 0 AND (SELECT state FROM supplier_bills WHERE id=v_bill) = 'open';
  ...
  INSERT INTO service_cases(case_number, customer_id, case_type, source, warranty_status, status)
    VALUES (v_pfx||'SC1', v_cust, 'customer_claim'::service_case_type,'customer'::service_case_source,
            'out_of_warranty'::service_case_warranty_status, 'triage'::service_case_status)
    RETURNING id INTO v_case;
  INSERT INTO service_cases(case_number, customer_id, case_type, source, warranty_status, status)
    VALUES (v_pfx||'SC2', v_cust, 'warranty'::service_case_type,'customer'::service_case_source,
            'in_warranty'::service_case_warranty_status,'triage'::service_case_status)
  ...
  v_pass := v_health ? 'ok' AND v_health ? 'findings' AND v_health->'summary' ? 'p0' AND v_health->'summary' ? 'p1' AND v_health->'summary' ? 'p2';
