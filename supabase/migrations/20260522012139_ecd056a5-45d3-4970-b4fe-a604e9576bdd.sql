-- Helper: ensure a delivery_route_orders row exists for a (route, schedule) pair
CREATE OR REPLACE FUNCTION public._ensure_route_order(_route uuid, _schedule uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF _route IS NULL OR _schedule IS NULL THEN RETURN NULL; END IF;
  SELECT id INTO v_id FROM delivery_route_orders
   WHERE schedule_id=_schedule AND status NOT IN ('cancelled','returned') LIMIT 1;
  IF v_id IS NOT NULL THEN
    IF (SELECT route_id FROM delivery_route_orders WHERE id=v_id) <> _route THEN
      UPDATE delivery_route_orders SET route_id=_route WHERE id=v_id;
    END IF;
    RETURN v_id;
  END IF;
  SELECT COALESCE(MAX(sequence)+1,1) INTO v_seq FROM delivery_route_orders WHERE route_id=_route;
  INSERT INTO delivery_route_orders(route_id, schedule_id, status, sequence)
  VALUES (_route, _schedule, 'planned', v_seq)
  RETURNING id INTO v_id;
  RETURN v_id;
END $$;

-- Update audit trigger to also keep delivery_route_orders in sync
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    IF NEW.route_id IS NOT NULL THEN
      SELECT vehicle_id INTO v_route_vehicle FROM delivery_routes WHERE id = NEW.route_id;
      IF v_picking IS NOT NULL THEN
        UPDATE stock_pickings
           SET route_id = NEW.route_id,
               ...
               vehicle_id = COALESCE(NEW.vehicle_id, v_route_vehicle, vehicle_id)
         WHERE id = v_picking;
      END IF;
      PERFORM public._ensure_route_order(NEW.route_id, NEW.id);
    END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    IF (NEW.route_id IS DISTINCT FROM OLD.route_id
      ...
      IF v_picking IS NOT NULL THEN
        ...
        UPDATE stock_pickings
         ...
         WHERE id = v_picking;
      END IF;
      IF NEW.route_id IS NOT NULL AND NEW.status NOT IN ('cancelled','delivered','rescheduled') THEN
        PERFORM public._ensure_route_order(NEW.route_id, NEW.id);
      END IF;
    END IF;
    ...
    IF NEW.status IS DISTINCT FROM OLD.status THEN
      ...
      ELSIF NEW.status = 'cancelled' THEN
        UPDATE delivery_route_orders SET status='cancelled' WHERE schedule_id=NEW.id AND status='planned';
        PERFORM public.log_schedule_event(NEW.sale_order_id, v_picking,
...
-- Update schedule_picking_to_route to also ensure the route_order exists
...
BEGIN
  ...
  UPDATE public.stock_pickings
     SET route_id = _route,
         ...
         vehicle_id = COALESCE(r.vehicle_id, vehicle_id)
   WHERE id = _picking;
    ...
    IF v_so_id IS NOT NULL THEN
      ...
      ELSE
        INSERT INTO public.delivery_schedules(
          sale_order_id, partner_id, scheduled_date, route_id, zone_id, vehicle_id,
          status, physical_state, fulfillment_type, created_by
        )
        SELECT v_so_id, so.partner_id, r.route_date, _route, v_zone, r.vehicle_id,
               'confirmed', 'in_stock', 'delivery', auth.uid()
          FROM public.sale_orders so WHERE so.id = v_so_id
        RETURNING id INTO v_existing;
      END IF;

      PERFORM public._ensure_route_order(_route, v_existing);
    END IF;
  END IF;
...
-- Backfill: create delivery_route_orders for existing confirmed/scheduled schedules already linked to a route
INSERT INTO public.delivery_route_orders(route_id, schedule_id, status, sequence)
SELECT ds.route_id, ds.id, 'planned',
       COALESCE((SELECT MAX(sequence)+1 FROM delivery_route_orders dro2 WHERE dro2.route_id = ds.route_id), 1)
                + ROW_NUMBER() OVER (PARTITION BY ds.route_id ORDER BY ds.created_at) - 1
  FROM public.delivery_schedules ds
 WHERE ds.route_id IS NOT NULL
   AND ds.status NOT IN ('cancelled','delivered','rescheduled')
   AND NOT EXISTS (
     SELECT 1 FROM public.delivery_route_orders dro
      WHERE dro.schedule_id = ds.id AND dro.status NOT IN ('cancelled','returned')
   );