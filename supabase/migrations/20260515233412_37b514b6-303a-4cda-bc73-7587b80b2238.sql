DROP FUNCTION IF EXISTS public._test_phase13();
...
BEGIN
  ...
  INSERT INTO partners(name,is_customer) VALUES (pfx||'CUST') RETURNING id INTO partner;
  INSERT INTO partners(name,is_supplier) VALUES (pfx||'SUP')  RETURNING id INTO supplier;
  ...
  -- T10 integridade
  SELECT count(*) INTO v_int FROM stock_quants WHERE quantity<0 OR reserved_quantity<0 OR reserved_quantity>quantity;
  v_passed := v_int=0;
  tests := tests || jsonb_build_array(jsonb_build_object('name','T10.stock_integrity','passed',v_passed,
    'observed', jsonb_build_object('violations',v_int)));
  ...
  -- T12 trigger sale.confirmed regressão
  ...
  SELECT count(*) INTO v_int FROM module_events WHERE event_type='sale.confirmed' AND entity_id=so12;
  v_passed := v_int=1;
  ...
  DELETE FROM module_events WHERE entity_id IN (SELECT id FROM sale_orders WHERE name LIKE pfx||'%');
...
REVOKE ALL ON FUNCTION public._test_phase13() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._test_phase13() TO authenticated, service_role;