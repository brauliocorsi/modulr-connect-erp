...
DECLARE
  ...
  v_user uuid := '00000000-0000-0000-0000-0000000c0ffe'::uuid;
BEGIN
...
BEGIN
  ...
  -- PATCH P03: inject fake jwt sub so auth.uid() resolves inside triggers/RPCs.
  PERFORM set_config('request.jwt.claims', json_build_object('sub', v_user::text)::text, true);

  -- PATCH P12: grant minimum group required by _m3_is_logistics (gates delivery_route_cash_close).
  SELECT id INTO v_group_inventory_user FROM groups WHERE code='inventory_user' LIMIT 1;
  INSERT INTO user_groups(user_id, group_id)
    VALUES (v_user, v_group_inventory_user)
    ON CONFLICT DO NOTHING;

  -- cash_register + cash_session abertos manualmente (fixture de teste).
  INSERT INTO cash_registers(name, warehouse_id, active, user_id)
    VALUES (v_pfx||'REG', v_wh, true, v_user) RETURNING id INTO v_register;
  ...
  v_report := v_report || jsonb_build_object('id','SETUP','status','OK',
    'observed', format('register=%s session=%s user=%s group=inventory_user', v_register, v_session, v_user));
  v_ok := v_ok + 1;
