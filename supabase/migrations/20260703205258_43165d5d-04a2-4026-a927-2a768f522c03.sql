...
-- Restaura create_purchase_need limpa (sem instrumentação)
...
AS $function$
DECLARE _id uuid; _supplier uuid; _po_name text;
BEGIN
...
-- Teste de regressão do caminho canónico (versão final)
...
BEGIN
  ...
  -- (c) converter em PO em curso → guarda evita nova need
  ...
  -- (d) receber PO: injeta stock, desativa supply_link antiga, replaneia → reserva
