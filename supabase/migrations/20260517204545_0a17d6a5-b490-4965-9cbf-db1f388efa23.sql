...
BEGIN
  ...
  scenario:='01_manual_need_creates_po'; passed:= v_po IS NOT NULL AND v_pol IS NOT NULL;
  detail:='po='||COALESCE(v_po::text,'NULL'); RETURN NEXT;

  SELECT variant_id, quantity INTO v_pol_variant, v_pol_qty FROM purchase_order_lines WHERE id=v_pol;
  scenario:='02_variant_preserved_on_po_line'; passed:= v_pol_variant=v_var01 AND v_pol_qty=10;
  detail:='var='||COALESCE(v_pol_variant::text,'NULL')||' qty='||v_pol_qty; RETURN NEXT;
  ...
  BEGIN v_res:=public.purchase_needs_create_po(ARRAY[v_need2], NULL, NULL); v_err:='no_error';
  ...
  BEGIN
    ...
    BEGIN v_res:=public.purchase_needs_create_po(ARRAY[v_need3], NULL, NULL); v_err:='no_error';
    EXCEPTION WHEN OTHERS THEN v_err:=SQLERRM; END;
  ...
  BEGIN
    ...
    scenario:='09_forced_supplier_overrides_suggested'; passed:= v_pol_partner=v_partner_b;
    detail:='po_partner='||COALESCE(v_pol_partner::text,'NULL'); RETURN NEXT;
  END;
  ...
  BEGIN
    ...
    scenario:='10_mo_link_preserved_on_need'; passed:= v_keep_link=v_mo;
  ...
  BEGIN
    ...
    INSERT INTO stock_quants(product_id, variant_id, location_id, quantity)
      VALUES (v_prod, v_var01, v_main_loc, 50) ON CONFLICT DO NOTHING;
    ...
    SELECT id INTO v_pn_e2e FROM purchase_needs
     WHERE manufacturing_order_id=v_mo AND product_id=v_prod LIMIT 1;
    ...
    scenario:='12_e2e_pol_variant02'; passed:= v_pol_variant=v_var02;
    detail:='pol_var='||COALESCE(v_pol_variant::text,'NULL'); RETURN NEXT;

    PERFORM public.confirm_purchase_order(v_po_e2e);
    SELECT id INTO v_picking FROM stock_pickings
      WHERE origin=(SELECT name FROM purchase_orders WHERE id=v_po_e2e) ORDER BY created_at DESC LIMIT 1;
    SELECT variant_id INTO v_sm_variant FROM stock_moves
      WHERE picking_id=v_picking AND purchase_order_line_id=v_pol_e2e LIMIT 1;
    scenario:='13_e2e_stock_move_variant02'; passed:= v_sm_variant=v_var02;
    detail:='sm_var='||COALESCE(v_sm_variant::text,'NULL'); RETURN NEXT;

    scenario:='14_e2e_stock_move_links_need';
    passed:= EXISTS(SELECT 1 FROM stock_moves WHERE picking_id=v_picking
                    AND purchase_order_line_id=v_pol_e2e AND purchase_need_id=v_pn_e2e);
    detail:= CASE WHEN passed THEN 'linked' ELSE 'missing' END; RETURN NEXT;

    -- Receive: prefill quantity_done so validate_picking lands stock at destination
    UPDATE stock_moves SET quantity_done=quantity
     WHERE picking_id=v_picking AND state<>'cancelled' AND COALESCE(quantity_done,0)=0;
    ...
    scenario:='15_e2e_stock_quant_variant02'; passed:= v_qty_v02 >= 6; detail:='qty_v02='||v_qty_v02; RETURN NEXT;
    ...
    SELECT qty_reserved INTO v_qty_reserved FROM mo_components
     WHERE mo_id=v_mo AND product_id=v_prod AND variant_id=v_var02 LIMIT 1;
    ...
    passed:= NOT EXISTS(SELECT 1 FROM mo_components
    ...
    scenario:='19_e2e_no_duplicates'; passed:= v_pn_count=1 AND v_po_count=1;
    detail:='pn='||v_pn_count||' pol='||v_po_count; RETURN NEXT;

    SELECT count(*) INTO v_negative_count FROM stock_quants WHERE product_id=v_prod AND quantity<0;
    scenario:='20_e2e_no_negative_stock'; passed:= v_negative_count=0; detail:='negative='||v_negative_count; RETURN NEXT;
  END;
  RETURN;
END $function$;
