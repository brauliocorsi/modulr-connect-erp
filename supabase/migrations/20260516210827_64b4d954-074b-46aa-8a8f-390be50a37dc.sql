...
BEGIN
  ...
  INSERT INTO public.loading_docks(name,stock_location_id) VALUES (v_prefix||'_DOCK', v_loc_veh);
  ...
  -- T1: PO receipt good
  INSERT INTO public.stock_pickings(name,kind,state,source_location_id,destination_location_id,scheduled_date)
  ...
  -- T2: PO receipt para customer location não dispara
  ...
  BEGIN
    INSERT INTO public.manufacturing_orders(product_id,quantity,state,warehouse_id,company_id)
    ...
    INSERT INTO public.stock_pickings(name,kind,state,source_location_id,destination_location_id,origin,scheduled_date)
  ...
  -- T4: Return good release dispara
  DECLARE v_pkg uuid;
  BEGIN
  ...
  -- T8: ajuste positivo interno dispara
  ...
  -- T9: ajuste em customer location não dispara
  DECLARE v_adj2 uuid;
  BEGIN
    INSERT INTO public.inventory_adjustments(name,location_id,state,company_id)
  ...
  -- T24: nenhum stock_move extra (criámos 3)
  ...
  -- T25: hook events sem erro
  v_assert_ok := EXISTS (SELECT 1 FROM public.allocation_hook_events WHERE status='ok')
              AND NOT EXISTS (SELECT 1 FROM public.allocation_hook_events WHERE status='error');
  v_tests := v_tests || jsonb_build_array(jsonb_build_object('test','25_hook_events_no_error','ok',v_assert_ok));
  IF v_assert_ok THEN v_passed:=v_passed+1; ELSE v_failed:=v_failed+1; END IF;
