...
cur.execute("SELECT _test_phase5()")
res=cur.fetchone()[0]; asserts=res["asserts"]; fails=0
rep=["# Phase 5 — Cancellation cascade report",f"_run: {datetime.datetime.utcnow().isoformat()}_","",
     "| # | Step | OK | Observed |","|---|---|---|---|"]
...
with open("/mnt/documents/phase5-cancellation-report.md","w") as f: f.write(out)
print("\n"+out)
